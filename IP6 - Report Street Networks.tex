...
\begin{enumerate}
    ...
    \item All graph points are assigned to there nearest centroids.
    \item The centroids are moved to the center of their assigned graph points.
    \item This process is repeated (from 2.) till no centroids are moved.
\end{enumerate}
...
Our implementation in CPlan\ref{CPlan} has produced the following image\ref{fig:KmeansGenerated}. The edges in a cluster are marked by a colour, the transitions between the found clusters are marked black.
\begin{figure}[!h]
    \centering
