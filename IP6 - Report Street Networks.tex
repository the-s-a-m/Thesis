...
To analyse and generate streets there are many approaches. First of all we learned how streets can be described in a grammar and if it would be possible to describe an existing street network based on a tree or graph. To separate a given street network into reasonable parts we tested many clustering algorithms. To evaluate the created clusters we used measurement methods provided by the architecture department of the ETH-Zurich. 
...
This Space Syntax grammar was defined 1976 by B. Hiller, A. Leaman, P. Stansall and M. Bedford in the paper "Space syntax" \citep{shapeGrammars:1972}. The grammar is a morphic  language and describes methods to analyse and generate urban areas and buildings. 
...
%TODO was copied!!
Space syntax uses a “graph theory” in quantifying axial maps. In space syntax, each
axial line is treated as a “node” (or space) in a graph and it is connected to its other adjacent axial lines by links. Then, a justified graph in space syntax is “a graph restructured so that a specific space is placed at the bottom, “the root space.” All spaces one syntactic step away from [the] root space [spaces that can be reached with one turn from the root] are put on the first level above, all spaces two steps away on the second level, etc . . . [it] offer[s] a visual picture of the overall depth of a lay-out seen from one of its points” (Klarqvist, 1993, p. 11).
...
%TODO v v v v v v v v v v v
\textbf{ADD REFERENCES HERE}

\subsubsection{Reduction Formula}
The reduction formula is used to determine the distance between two clusters. It is called reduction formula, because the problem to obtain this distance is reduced to the problem to get the distance between two nodes.
...
With clusters $C_1$ and $C_2$, which are sets of nodes and the distance function $d(i, j)$, which determines the distance between nodes $i$ and $j$.
