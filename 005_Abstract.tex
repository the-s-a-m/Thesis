\begin{abstract}
    In this thesis different cluster analysis algorithms where compared and extended on a graph representation of a street networks.
    ...
    During tests with big street networks high memory usage was a problems. As a result three optimisations where made. First, float precision was used. Second, a data structure witch does not store value twice was developed. Third the resulting cluster distances where saved at the position of the resource positions. 
    
    To rate the created clusters many graph analysis parameters where implemented. The values can be compared and exported into a JSON-file for later use.
    
    %TODO UPGMA/WPGMA entweder Arithmetic Mean oder Arithmetic-mean aber nicht beides!
\end{abstract}
