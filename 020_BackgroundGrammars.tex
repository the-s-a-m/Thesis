...
\acrshort{acr:CPlan} (section \ref{CPlan}) does handle the street networks as graphs. This results in the same measurements methods, which can be applied as mentioned for the point-based space syntax. The clustering algorithms --- later discussed in sections \ref{sec:K-Means} and \ref{sec:hierarchicalClustering} --- are applied on vertex / points and unidirectional edges between the vertices.